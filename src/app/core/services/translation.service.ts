...
import { SupportedLanguage, Translation, LanguageConfig } from '../interfaces/language.interface';
...
export class TranslationService {
  ...
  private defaultTranslations: Translation = {
    ...
    home: {
      ...
      description: "A modern cross-platform application built with Ionic and Angular"
    },
    about: {
      title: "About Us",
      description: "This is a modern cross-platform application built with Ionic Framework and Angular.",
      features: "Features",
  ...
  private loadStoredLanguage(): void {
    const storedLanguage = localStorage.getItem(this.languageKey) as SupportedLanguage;
  ...
  private loadTranslations(): void {
    ...
    const translationRequests = this.supportedLanguages.map(lang =>
      this.http.get<Translation>(`assets/i18n/${lang.code}.json`).pipe(
        map(translations => ({ language: lang.code, translations }))
      )
    );
    ...
    forkJoin(translationRequests).subscribe({
      next: (results) => {
        results.forEach(result => {
          this.translations[result.language as SupportedLanguage] = result.translations;
        });
        // Clear cache to refresh all translations
